...
class RTCFireSession {
  ...
  private maybeProcessParticipantChanges() {
    ...
    const set = new Set(this._participants);
    ...
    for (const existingId in this.participantInfo) {
  ...
  set muted(muted) {
    this._muted = muted;
    if (this.localStream) {
      for (const t of this.localStream
        .getTracks()
  ...
  private onAddParticipant(pid) {
    ...
    const offerer = !!this.options.isOfferer(pid);
    ...
    info.writeRefOnDisconnect = onDisconnect(writeRef);
    set(info.writeRefOnDisconnect, null);
  }
  ...
  private onRemoveParticipant(pid) {
    ...
    const { onReadValueChange, readRef, writeRef, writeRefOnDisconnect } =
  ...
  private initConnectionForParticipant(pid) {
    ...
    const info = this.participantInfo[pid];
    const { offerer, writeRef } = info;
    ...
    info.conn.onicecandidate = (ev) => {
      ...
      const cKey = hashString(`${c.candidate}|${c.sdpMLineIndex}`);
  ...
  private processNewIceCandidates(pid) {
    const info = this.participantInfo[pid];
    const { conn, readVal, processedIceCandidates } = info;
    const {
      iceCandidates,
    ...
    for (const { sdpMLineIndex, candidate } of Object.values(iceCandidates)) {
      const cKey = `${sdpMLineIndex}|${candidate}`;
  ...
  private async beginNegotiation(pid) {
    ...
    const sessionDescription = await info.conn.createOffer();
    info.conn.setLocalDescription(sessionDescription);
    await update(writeRef, { offer: sessionDescription });
  }
  ...
  private async maybeContinueNegotiation(pid) {
    const info = this.participantInfo[pid];
    const { conn, negotiationState, writeRef, readVal } = info;
    ...
    switch (negotiationState) {
      ...
      case NegotiationState.AnswererWaitingForOffer: {
        ...
        if (offer) {
          ...
          const sessionDescription = await conn.createAnswer();
          conn.setLocalDescription(sessionDescription);
          await update(writeRef, { answer: sessionDescription });
        }
        break;
      }

      case NegotiationState.OffererWaitingForAnswer: {
        const { answer } = readVal;
        if (answer) {
          info.negotiationState = NegotiationState.WaitingToFinishIceCandidates;
